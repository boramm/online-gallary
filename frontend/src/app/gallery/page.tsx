...
function GalleryContent() {
  ...
  const { isAdmin, checkAdminStatus, logout } = useAdmin();
  ...
  // 관리자 로그인 처리 (URL 파라미터 체크)
  useEffect(() => {
    // window.location.search 직접 확인
    console.log("[Gallery] window.location.search:", window.location.search);
    const urlParams = new URLSearchParams(window.location.search);
    const adminPassword = urlParams.get("admin") || searchParams.get("admin");
    
    console.log("[Gallery] Admin param from URL:", adminPassword);
    
    if (adminPassword === "syu3454") {
      console.log("[Gallery] Admin param found:", adminPassword);
      // localStorage에 저장
      localStorage.setItem("adminMode", "true");
      console.log("[Gallery] Saved to localStorage: adminMode = true");
      alert("관리자 모드 활성화");
  ...
  }, [searchParams, checkAdminStatus, router]);
  ...
  const loadDepartments = async () => {
    try {
      ...
      const response = await fetch(`${API_URL}/photos/departments`);
      if (response.ok) {
  ...
  const loadPhotos = async (pageNum: number, reset = false) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/photos?${params.toString()}`
      );
        ...
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            ...
            {isAdmin && (
              <button
                onClick={logout}
                className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-red-500/20 hover:bg-red-500/30 text-red-600 dark:text-red-400 font-semibold transition-colors text-sm"
                title="클릭하면 관리자 모드 종료"
              >
                🔧 관리자
              </button>
            )}
